# Contributing to Bitcore-Safe

We're working hard to make *bitcore-safe* the most powerful JavaScript library for working with Safe. Our goal is to have *bitcore-safe* be a library that can be used by anyone interested in Safe, and to level expertise differences with great design and documentation.
...
These are some global design goals in bitcore-safe to which any change should adhere.
...
```sh
git remote add safe git@github.com:safeevo/bitcore-safe.git
git pull --rebase safe master
```
...
```
Finally go to [github.com/safeevo/bitcore-safe](https://github.com/safeevo/bitcore-safe) in your web browser and issue a new pull request.
